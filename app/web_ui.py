...
def build_answer(query: str, context: str, history: list[dict]) -> str:
    ...
    if has_expired_fallback:
        ...
        length_guide = (
            "아래 예시 형식으로만 답변하세요 (각 항목 한 줄):\n"
            "- [카테고리] 공고명: 마감 X.X / 핵심 조건\n"
            "• 마감일이 없거나 상시 모집인 경우: 마감 자리에 '상시'로 표기할 것.\n"
            "좋은 예: '- [대학공지] 인터랙션 연구실 하계인턴 모집: 마감 5.29 / 4학년 학부생'\n"
            "좋은 예(상시): '- [청년정책] 국민취업지원제도: 상시 / 취업 취약계층'\n"
            "나쁜 예(절대 금지): 업무 내용 목록, 자격 조건 나열, 추가 설명 문단, 마감일 없는 항목에 임의 날짜 생성.\n"
            "제공된 모든 유효 문서를 각각 한 줄씩 나열할 것 (1~2개만 선별하지 말 것). "
    ...
    # has_expired_fallback은 early return으로 이미 처리됨 → 후처리 불필요
