...
RAGAS Answer Relevancy는 두 버전 간 차이가 0.003으로 미미하여 변별력이 낮습니다. 이를 보완하기 위해 생성 답변과 정답(ground truth)을 동일한 임베딩 모델로 직접 인코딩한 후 코사인 유사도를 측정했습니다. LLM 평가자를 거치지 않고 의미적 일치도를 직접 수치화하므로 두 모델의 실질적인 답변 품질 차이를 더 명확하게 드러냅니다.
